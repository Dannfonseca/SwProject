...
export const expandCollabSearch = (term: string) => {
  const entry = getCollabEntry(term);
  if (!entry) return null;
  ...
  entry.variants.forEach((name, index) => {
    if (!name) return;
    addName(name);
    const element = ELEMENTS[index];
    if (element) {
      addName(`${element} ${name}`);
      if (element === 'Light' && name === 'White Tiger Blade Master') {
        addName('L.W.T. Blade Master');
        addName('Light White Tiger Blade Master');
      }
    }
  });

  addName(entry.collab);
  return {
    collab: entry.collab,
    variants: entry.variants,
    names: Array.from(expanded)
  };
};
